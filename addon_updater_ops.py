...
def update_settings_ui(self, context, element=None):
    ...
    row = box.row()
    row.alignment = 'LEFT'
    row.prop(settings, 'updater_expand_prefs', text="Updates",
             icon="TRIA_DOWN" if settings.updater_expand_prefs else "TRIA_RIGHT", emboss=False)
    ...
    if settings.updater_expand_prefs:
        ...
        if updater.auto_reload_post_update == False:
            saved_state = updater.json
            if "just_updated" in saved_state and saved_state["just_updated"] == True:
                row.label(text="Restart blender to complete update",
                          icon="ERROR")
                return
        ...
        if updater.manual_only == False:
            ...
            col.operator(addon_updater_restore_backup.bl_idname,
                         text=backuptext)
...
def register(bl_info):
    ...
    updater.website = "https://github.com/Poly-Haven/HAT"
